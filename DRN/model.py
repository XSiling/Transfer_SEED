...
class Classifier(nn.Module):
    def __init__(self):
        ...
        self.class_classifier.add_module('c_fc1', nn.Linear(EXTRACTED_FEATURE_DIM + 1, 100))
...
class DANN(nn.Module):
    ...
    def forward(self, input_data, alpha=1, source=True): # input_data: (batch_size, ori_dim(310))
        ...
        class_output = self.classifier(torch.cat((feature, domain_pred), dim=1)) # (batch_size, 4)
    ...
    def get_adversarial_result(self, x, source=True, alpha=1): # x: (batch_size, EXTRACTED_FEATURE_DIM)
        ...
        loss_adv = loss_fn(domain_pred, domain_label.float().view(domain_label.shape[0], -1))
